# R106-B Official Pal Skills Index Batch 1 Boundary Eval

Date: 2026-06-28

## Scope

This eval covers the R106-B skills index backfill for:
...
It verifies index completeness and boundary preservation. It does not execute Pal Skills, Runtime Agent Skills, tests, scanners, validators, connectors, plugin discovery, or external business-system probes.

## Required checks

| Check | Expected result |
| --- | --- |
| Selected skills indexes exist | Atlas, Quinn, and Morgan each have `skills/index.md` |
| Required headings exist | Every selected index includes the R106-B required heading set |
| Pal Skill definition exists | Each index defines Pal Skills as role-level work capabilities |
| Agent Skill boundary exists | Each index states that Runtime Agent Skills should be referenced, not copied |
| Current assets are listed | Existing flat skill cards, directory skill packages, and support files are listed |
| Candidate skills are separated | No new concrete Pal Skill is added in R106-B; candidates are marked `not-run` / needs-review |
| Related workflows and runbooks are referenced | Existing workflow/runbook indexes or directories are referenced |
| Verification boundary exists | Each index requires current runtime evidence before execution claims |
| Memory writeback boundary exists | Each index prevents reports from being written wholesale into memory |
...
The batch fails if any selected index:

- treats a Runtime Agent Skill as a Pal Skill;
- claims a Pal Skill executes commands, calls APIs, changes files, or performs runtime checks by itself;
